...
$(document).ready(function () {
  ...
  $(".main-total-comparisions").text(
    totalComparisons + (totalComparisons === 1 ? " Car Added" : " Cars Added")
  );
  ...
  $(document).on("click", ".btnCompare2", function () {
    ...
    const vehicleData = $(this).attr("data-vehicle");

    if (!vehicleId || !vehicleData) {
      console.error("Vehicle ID or data is missing.");
      return;
    }
    ...
    const carObj = {
      ...
      selected: false,
      text: vehicle.name,
      _resultId: "select2-state-" + vehicle.id,
    };

    let comparisons = JSON.parse(localStorage.getItem("comparisions")) || {};
    const comparisonKeys = Object.keys(comparisons);

    const existingKey = Object.keys(comparisons).find(
      (key) => comparisons[key].id === vehicleId
    );

    if (existingKey) {
      delete comparisons[existingKey];

      const updatedComparisons = {};
      let index = 1;
      Object.values(comparisons).forEach((vehicle1) => {
        const newKey = getOrdinalKey(index++);
        updatedComparisons[newKey] = vehicle1;
      });

      localStorage.setItem("comparisions", JSON.stringify(updatedComparisons));
      $(this).removeClass("active");
      $(".total-comparisions").text(Object.keys(updatedComparisons).length);

      console.log("Item removed and keys shifted:", updatedComparisons);
      return;
    }

    if (comparisonKeys.length >= 6) {
      alert("Cannot add more than 6 items to the comparison list.");
      return;
    }

    const nextOrdinal = getOrdinalKey(comparisonKeys.length + 1);
    comparisons[nextOrdinal] = carObj;
    localStorage.setItem("comparisions", JSON.stringify(comparisons));
    console.log("Updated Comparisons:", comparisons);
    $(".total-comparisions").text(Object.keys(comparisons).length);
    $(this).addClass("active");
  });
...
$(window).on("load", function () {
  ...
  $(".accordiantitle").on("click", function () {
    ...
    $(this).toggleClass("active");
    $(this)
      .parent(".eachaccordian")
      .find(".accordiancontent")
      .toggleClass("active");
  });
  ...
  $(".comparecartabsbar a").on("click", function (e) {
    ...
    var targetOffset =
      $('.eachaccordian[data-accordsection="' + scrolltoaccord + '"]').offset()
        .top - scrollOffset;
    ...
    $('.eachaccordian[data-accordsection="' + scrolltoaccord + '"]')
      .find(".accordiancontent")
      .addClass("active");
    ...
    if ($(this).find("input").is(":checked")) {
      ...
      $(".comparecaraccordians .tableinnercols").each(function (index) {
        if (
          ...
          $(this).children().eq(0).text() != $(this).children().eq(2).text()
        ) {
          $(this).addClass("greenhighlight");
        } else if (
          $(this).children().eq(3).text() &&
          $(this).children().eq(0).text() != $(this).children().eq(3).text()
        ) {
          $(this).addClass("greenhighlight");
        } else if (
          $(this).children().eq(4).text() &&
          $(this).children().eq(0).text() != $(this).children().eq(4).text()
        ) {
          $(this).addClass("greenhighlight");
        } else if (
          $(this).children().eq(5).text() &&
          $(this).children().eq(0).text() != $(this).children().eq(5).text()
        ) {
          $(this).addClass("greenhighlight");
        }
      });
  ...
  $(".select-vehicle").on("click", function () {
    ...
    if ($children.filter(":hidden").length === 0) {
      alert(
        "Max cars have been reached. Remove a car or start a new comparison to continue."
      );
    }
    let carShown = false;
  ...
  function updateSlider() {
    ...
    $(".carslidernavigation .next")
      ...
      .on("click", function () {
        ...
        if (lastVisible + 1 < cards.length) {
          cards
            .slice(firstVisible, firstVisible + cardsToShow)
            .addClass("hide");
          cards
            .slice(lastVisible + 1, lastVisible + 1 + cardsToShow)
            .removeClass("hide");
        }
    ...
    $(".carslidernavigation .prev")
      ...
      .on("click", function () {
        ...
        if (firstVisible > 0) {
          cards
            .slice(lastVisible - cardsToShow + 1, lastVisible + 1)
            .addClass("hide");
          cards
            .slice(firstVisible - cardsToShow, firstVisible)
            .removeClass("hide");
        }
...
function SaveDataToLocalStorageWithoutKey(vehicle) {
  ...
  if (Object.keys(local_data).length + 1 <= 6) {
    comparisions[getNumberWithOrdinal(Object.keys(local_data).length + 1)] =
      vehicle.toString();
    localStorage.setItem("comparisions", JSON.stringify(comparisions));
    $(".total-comparisions").html(
      Object.keys(JSON.parse(localStorage.getItem("comparisions"))).length
    );
  } else {
    alert(
      "Max cars have been reached and to remove a car or start a new comparison to continue."
    );
  }
...
function SaveComparisionsDataToLocalStorage(vehicle, key) {
  ...
  var comparisions =
    JSON.parse(localStorage.getItem("query_comparisions")) || {};
...
function DeleteComparisionsDataFromLocalStorage(key) {
  ...
  if (Object.keys(comparisions).length > 0) {
    if (comparisions.hasOwnProperty(key)) {
      ...
      $(".total-comparisions").text(
        totalComparisons + (totalComparisons === 1 ? " Car" : " Cars")
      );
    } else {
...
function makeFavourite(vehicle_id, user_id) {
  ...
  $.post(
    ...
    function (result) {
      if (
        result.success &&
        result.success === "Vehicle marked favourite successfully."
      ) {
        $("#favourite-" + vehicle_id + " ul li a.favourite-vehicle").css(
          "color",
          "red"
        );
        $("#favourite-" + vehicle_id + " a.favourite-vehicle i").css(
          "color",
          "red"
        );
        $(".favourite-vehicle.detail").html(
          '<img src="/frontend/assets/images/heart-icon-filled.png" alt="">'
        );
      } else if (
        result.success &&
        result.success === "Vehicle removed from favourite successfully."
      ) {
        $("#favourite-" + vehicle_id + " ul li a.favourite-vehicle").css(
          "color",
          "#fff"
        ...
        $(".favourite-vehicle.detail").html(
          '<img src="/frontend/assets/images/heart-icon.png" alt="">'
        );
      }
    }
  );
...
function priceWithCommas(value) {
  ...
  let formattedPrice = intValue.toLocaleString("en-US", {
    style: "currency",
    currency: "USD",
  });
...
$(document).ready(function () {
  ...
  $(".vehicledelete").on("click", function () {
    ...
    currentState
      ...
      .attr(
        "src",
        siteurl + "frontend/assets/images/comparision-placeholder.jpeg"
      );
  ...
  function DeleteDataFromLocalStorage(key) {
    ...
    if (Object.keys(comparisions).length <= 6) {
      if (comparisions.hasOwnProperty(key)) {
        ...
        $(".main-total-comparisions").text(
          totalComparisons +
            (totalComparisons === 1 ? " Car Added" : " Cars Added")
        );
        $(".total-comparisions").text(totalComparisons);
      } else {
        console.log("Key does not exist. Unable to delete.");
        ...
        .on("change", function (e) {
          ...
          if (selectedData.length > 0) {
            ...
            if (name !== "Select Vehicle") {
              let vehicle_link = `<a href="${siteurl}vehicle/${ID}" target="_blank">${name.substring(
                0,
                35
              )}${name.length > 35 ? "..." : ""}</a>`;
              ...
              currentState
                ...
                .attr(
                  "onclick",
                  `window.location.href='${siteurl}vehicle/${ID}'`
                );
              currentState
                .find(".social-share-links a")
                .eq(0)
                .attr(
                  "onclick",
                  `window.open('https://www.facebook.com/sharer/sharer.php?u=${siteurl}vehicle/${ID}')`
                );
              currentState
                .find(".social-share-links a")
                .eq(1)
                .attr(
                  "onclick",
                  `window.open('https://twitter.com/intent/tweet?url=${siteurl}vehicle/${ID}')`
                );
              currentState
                .find(".social-share-links a")
                .eq(2)
                .attr(
                  "onclick",
                  `window.open('mailto:?subject=Check%20out%20this%20vehicle&body=${siteurl}vehicle/${ID}')`
                );
              currentState
                .find(".favourite-vehicle")
                .attr("onclick", `makeFavourite('${ID}', '${auth_user_id}')`);
              currentState
                .find(".favourite-vehicle")
                .parent()
                .attr("id", `favourite-${ID}`);
            }
            ...
            if (rankTag) {
              currentState
                .find(".favourite-vehicle i")
                .css("color", carFullData?.user?.length > 0 ? "red" : "");
              vehicleDetail(
                JSON.parse(carFullData.data),
                rankTag.slice(0, 1) - 1,
                currentState,
                name,
                ID,
            ...
            ["horse-power", "rpm", "torque", "torque-rpm"].forEach((spec) => {
              ...
              $(`.specifications-${spec}`)
                ...
                .children()
                .each(function () {
                  let currentSpec = parseInt(
                    $(this).find(".valueholder").text()
                  );
                  $(this)
                    .find(".horsepowermeter")
                    .children()
                    .css(
                      "width",
                      currentSpec >= maxSpec
                        ? "100%"
                        : `${(currentSpec * 100) / maxSpec}%`
                    );
                });
            });
          } else {
  ...
  const storedComparisons = localStorage.getItem("comparisions")
    ? JSON.parse(localStorage.getItem("comparisions"))
    : {};
  ...
  function initializeComparisionSelectors(comparisons) {
    ...
    Object.entries(comparisons).forEach(([rank, carObj], index) => {
      if (index < selectors.length) {
        ...
        $(selector)
          .find(".favourite-vehicle i")
          .css("color", carObj?.user?.length > 0 ? "red" : "");
...
function vehicleDetail(vehicle, index, currentState, name, ID, data) {
  ...
  if (name) $(".carcomparisionlists").children().eq(index).text(name);
  if (currentState) {
  ...
  $(".stickycol.specifications-body")
    .next()
    .children()
    .eq(index)
    .text(data?.body_type);
  $(".stickycol.specifications-trim")
    .next()
    .children()
    .eq(index)
    .text(vehicle.style?.trim ?? vehicle.style?.nameWoTrim);
  $(".stickycol.specifications-cylinders")
    .next()
    .children()
    .eq(index)
    .text(vehicle.engine?.cylinders);
  $(".stickycol.specifications-drive-train")
    .next()
    .children()
    .eq(index)
    .text(vehicle.style?.drivetrain);
  $(".stickycol.specifications-fuel-economy-city")
    .next()
    .children()
    .eq(index)
    .text(
      vehicle.engine.fuelEconomy ? vehicle.engine.fuelEconomy?.city?.low : "-"
    );
  $(".stickycol.specifications-fuel-economy-highway")
    .next()
    .children()
    .eq(index)
    .text(
      vehicle.engine.fuelEconomy ? vehicle.engine.fuelEconomy.hwy?.low : "-"
    );
  $(".stickycol.specifications-fuelType")
    .next()
    .children()
    .eq(index)
    .text(data?.fuel_type?.name);
  $(".stickycol.specifications-engineType")
    .next()
    .children()
    .eq(index)
    .text(data?.engine_type?.name ?? vehicle?.engine?.engineType?._);
  // $('.stickycol.specifications-torque').next().children().eq(index).text(vehicle.engine?.netTorque?.value);
  $(".stickycol.specifications-size")
    .next()
    .children()
    .eq(index)
    .text(
      vehicle?.engine.fuelCapacity
        ? vehicle?.engine?.fuelCapacity?.high +
            vehicle.engine?.fuelCapacity?.unit
        : "-"
    );
  $(".stickycol.specifications-stock-number")
    .next()
    .children()
    .eq(index)
    .text(vehicle?.style?.acode?._);
  $(".stickycol.specifications-displacements")
    .next()
    .children()
    .eq(index)
    .text(
      vehicle?.engine.length > 0 &&
        vehicle.engine[0].displacement.value.length > 0
        ? vehicle.engine[0]?.displacement?.value[0]?._ +
            vehicle?.engine[0]?.displacement?.value[0]?.unit
        : vehicle?.engine.displacement
        ? vehicle?.engine?.displacement?.value?._ +
          vehicle?.engine?.displacement?.value?.unit
        : "-"
    );

  //horse power
  if (vehicle.engine.length > 0) {
    $(".stickycol.specifications-horse-power")
      .next()
      .children()
      .eq(index)
      .html(
        '<div class="horsepowermeter" data-metervalue="0"><div class="overlaymeter"></div><div class="valueholder">' +
          vehicle.engine[0].horsepower?.value +
          "</div></div>"
      );
    $(".stickycol.specifications-horse-power")
      .next()
    ...
    $(".stickycol.specifications-horse-power")
      ...
      .html(
        '<div class="horsepowermeter" data-metervalue="0"><div class="overlaymeter"></div><div class="valueholder">' +
          vehicle.engine?.horsepower?.value +
          "</div></div>"
      );
    $(".stickycol.specifications-horse-power")
      .next()
  ...
  //RPM
  if (vehicle.engine.length > 0) {
    $(".stickycol.specifications-rpm")
      .next()
      .children()
      .eq(index)
      .html(
        '<div class="horsepowermeter" data-metervalue="0"><div class="overlaymeter"></div><div class="valueholder">' +
          vehicle.engine[0].horsepower?.rpm +
          "</div></div>"
      );
    $(".stickycol.specifications-rpm")
      .next()
  ...
  //net torque
  if (vehicle?.engine?.length > 0) {
    $(".stickycol.specifications-torque")
      .next()
      .children()
      .eq(index)
      .html(
        '<div class="horsepowermeter" data-metervalue="0"><div class="overlaymeter"></div><div class="valueholder">' +
          vehicle.engine[0].netTorque?.value +
          "</div></div>"
      );
    $(".stickycol.specifications-torque")
      .next()
    ...
    $(".stickycol.specifications-torque")
      ...
      .html(
        '<div class="horsepowermeter" data-metervalue="0"><div class="overlaymeter"></div><div class="valueholder">' +
          vehicle.engine?.netTorque?.value +
          "</div></div>"
      );
    $(".stickycol.specifications-torque")
      .next()
  ...
  //net torque
  if (vehicle?.engine?.length > 0) {
    $(".stickycol.specifications-torque-rpm")
      .next()
      .children()
      .eq(index)
      .html(
        '<div class="horsepowermeter" data-metervalue="0"><div class="overlaymeter"></div><div class="valueholder">' +
          vehicle.engine[0].netTorque?.rpm +
          "</div></div>"
      );
    $(".stickycol.specifications-torque-rpm")
      .next()
  ...
  $(".stickycol.dimension-invoice")
    .next()
    .children()
    .eq(index)
    .text(
      priceWithCommas(vehicle?.basePrice?.invoice?.low) +
        " / " +
        priceWithCommas(vehicle?.basePrice?.msrp?.high)
    );
    ...
    for (let i = 0; i < vehicle.genericEquipment.length; i++) {
      if (Object.hasOwn(vehicle.genericEquipment[i], "definition")) {
        if (Object.hasOwn(vehicle.genericEquipment[i].definition, "category")) {
          ...
          if (
            vehicle.genericEquipment[i].definition.category._ == "Leather Seats"
          )
            $(".stickycol.seating-leather-seats")
              .next()
              .children()
              .eq(index)
              .html(
                vehicle.genericEquipment[i].definition?.header?._
                  ? '<i class="fa fa-check"></i>'
                  : "-"
              );

          if (
            vehicle.genericEquipment[i].definition.category._ == "Seat Memory"
          )
            $(".stickycol.seating-seat-memory")
              .next()
              .children()
              .eq(index)
              .html(
          ...
          if (
            vehicle.genericEquipment[i].definition.category._ ==
            "Heated Front Seat(s)"
          )
            $(".stickycol.seating-heated-front-seats")
              .next()
              .children()
              .eq(index)
              .html(
          ...
          if (
            ...
            $(".stickycol.seating-heated-rear-seats")
              .next()
              .children()
              .eq(index)
              .html(
          ...
          if (
            ...
            $(".stickycol.seating-cooled-front-seats")
              .next()
              .children()
              .eq(index)
              .html(
          ...
          if (
            vehicle.genericEquipment[i].definition.category._ == "AM/FM Stereo"
          )
            $(".stickycol.accessories-stereo")
              .next()
              .children()
              .eq(index)
              .html(
          ...
          if (
            ...
            $(".stickycol.accessories-climate-control")
              .next()
              .children()
              .eq(index)
              .html(
          ...
          if (
            ...
            $(".stickycol.accessories-security-system")
              .next()
              .children()
              .eq(index)
              .html(
          ...
          if (
            ...
            $(".stickycol.accessories-cruise-control")
              .next()
              .children()
              .eq(index)
              .html(
          ...
          if (
            vehicle.genericEquipment[i].definition.category._ == "Keyless Entry"
          )
            $(".stickycol.accessories-keyless-entry")
              .next()
              .children()
              .eq(index)
              .html(
          ...
          if (
            ...
            $(".stickycol.accessories-power-door-locks")
              .next()
              .children()
              .eq(index)
              .html(
          ...
          if (
            ...
            $(".stickycol.accessories-heated-mirrors")
              .next()
              .children()
              .eq(index)
              .html(
          ...
          if (
            vehicle.genericEquipment[i].definition.category._ == "Sun/Moonroof"
          )
            $(".stickycol.accessories-sun-moonroof")
              .next()
              .children()
              .eq(index)
              .html(
          ...
          if (
            ...
            $(".stickycol.accessories-intermittent-wipers")
              .next()
              .children()
              .eq(index)
              .html(
          ...
          if (
            ...
            $(".stickycol.accessories-auto-dimming-rearview-mirror")
              .next()
              .children()
              .eq(index)
              .html(
          ...
          if (
            vehicle.genericEquipment[i].definition.category._ == "Luggage Rack"
          )
            $(".stickycol.accessories-luggage-rack")
              .next()
              .children()
              .eq(index)
              .html(
          ...
          if (
            ...
            $(".stickycol.accessories-bluetooth-connection")
              .next()
              .children()
              .eq(index)
              .html(
          ...
          if (
            ...
            $(".stickycol.accessories-back-up-camera")
              .next()
              .children()
              .eq(index)
              .html(
          ...
          if (
            vehicle.genericEquipment[i].definition.category._ == "Keyless Start"
          )
            $(".stickycol.accessories-keyless-start")
              .next()
              .children()
              .eq(index)
              .html(
          ...
          if (
            ...
            $(".stickycol.accessories-lane-departure-warning")
              .next()
              .children()
              .eq(index)
              .html(
          ...
          if (
            ...
            $(".stickycol.accessories-cruise-control-steering-assist")
              .next()
              .children()
              .eq(index)
              .html(
          ...
          if (
            ...
            $(".stickycol.accessories-smart-device-integration")
              .next()
              .children()
              .eq(index)
              .html(
          ...
          if (
            ...
            $(".stickycol.accessories-automatic-parking")
              .next()
              .children()
              .eq(index)
              .html(
          ...
          if (
            ...
            $(".stickycol.accessories-power-liftgate")
              .next()
              .children()
              .eq(index)
              .html(
          ...
          if (
            ...
            $(".stickycol.accessories-navigation-system")
              .next()
              .children()
              .eq(index)
              .html(
          ...
          if (
            ...
            $(".stickycol.accessories-panoramic-roof")
              .next()
              .children()
              .eq(index)
              .html(
          ...
          if (
            ...
            $(".stickycol.accessories-headsup-display")
              .next()
              .children()
              .eq(index)
              .html(
          ...
          if (
            ...
            $(".stickycol.accessories-accessories-cruise-control-assist")
              .next()
              .children()
              .eq(index)
              .html(
          ...
          if (
            ...
            $(".stickycol.accessories-cruise-control-adaptive")
              .next()
              .children()
              .eq(index)
              .html(
  ...
  if (vehicle.technicalSpecification) {
    for (let i = 0; i < vehicle.technicalSpecification.length; i++) {
      if (Object.hasOwn(vehicle.technicalSpecification[i], "definition")) {
        if (
          Object.hasOwn(vehicle.technicalSpecification[i].definition, "title")
        ) {
          //List Down All properties
          if (
            vehicle.technicalSpecification[i].definition.title._ ==
            "EPA Classification"
          )
            $(".stickycol.vehicle-epa-classification")
              .next()
              .children()
              .eq(index)
              .text(vehicle.technicalSpecification[i]?.value?.value);

          if (
            vehicle.technicalSpecification[i].definition.title._ ==
            "Passenger Capacity"
          )
            $(".stickycol.dimension-pasenger-capacity")
              .next()
              .children()
              .eq(index)
              .text(vehicle.technicalSpecification[i]?.value?.value);
          ...
          if (
            ...
            $(".stickycol.dimension-front-head-room")
              .next()
              .children()
              .eq(index)
              .text(vehicle.technicalSpecification[i]?.value?.value);
          ...
          if (
            ...
            $(".stickycol.dimension-front-leg-room")
              .next()
              .children()
              .eq(index)
              .text(vehicle.technicalSpecification[i]?.value?.value);
          ...
          if (
            ...
            $(".stickycol.dimension-front-shoulder-room")
              .next()
              .children()
              .eq(index)
              .text(vehicle.technicalSpecification[i]?.value?.value);
          ...
          if (
            ...
            $(".stickycol.seating-front-hip-room")
              .next()
              .children()
              .eq(index)
              .text(vehicle.technicalSpecification[i]?.value?.value);
          ...
          if (
            ...
            $(".stickycol.seating-second-head-room")
              .next()
              .children()
              .eq(index)
              .text(vehicle.technicalSpecification[i]?.value?.value);
          ...
          if (
            ...
            $(".stickycol.seating-second-leg-room")
              .next()
              .children()
              .eq(index)
              .text(vehicle.technicalSpecification[i]?.value?.value);
          ...
          if (
            ...
            $(".stickycol.seating-second-shoulder-room")
              .next()
              .children()
              .eq(index)
              .text(vehicle.technicalSpecification[i]?.value?.value);
          ...
          if (
            ...
            $(".stickycol.seating-second-hip-room")
              .next()
              .children()
              .eq(index)
              .text(vehicle.technicalSpecification[i]?.value?.value);
          ...
          if (
            ...
            $(".stickycol.seating-track-width-front")
              .next()
              .children()
              .eq(index)
              .text(vehicle.technicalSpecification[i]?.value?.value);
          ...
          if (
            ...
            $(".stickycol.seating-track-width-rear")
              .next()
              .children()
              .eq(index)
              .text(vehicle.technicalSpecification[i]?.value?.value);
          ...
          if (
            ...
            $(".stickycol.seating-ground-clearance")
              .next()
              .children()
              .eq(index)
              .text(vehicle.technicalSpecification[i]?.value?.value);
          ...
          if (
            ...
            $(".stickycol.seating-cargo-volume")
              .next()
              .children()
              .eq(index)
              .text(vehicle.technicalSpecification[i]?.value?.value);
          ...
          if (
            ...
            $(".stickycol.seating-cargo-volume-trunk")
              .next()
              .children()
              .eq(index)
              .text(vehicle.technicalSpecification[i].range.max);
          ...
          if (
            ...
            $(".stickycol.chasis-dead-max-trailor")
              .next()
              .children()
              .eq(index)
              .text(vehicle.technicalSpecification[i]?.value?.value);
          ...
          if (
            ...
            $(".stickycol.chasis-dead-max-tongue")
              .next()
              .children()
              .eq(index)
              .text(vehicle.technicalSpecification[i]?.value?.value);
          ...
          if (
            ...
            $(".stickycol.chasis-wt-max-trailor")
              .next()
              .children()
              .eq(index)
              .text(vehicle.technicalSpecification[i]?.value?.value);
          ...
          if (
            ...
            $(".stickycol.chasis-wt-max-tongue")
              .next()
              .children()
              .eq(index)
              .text(vehicle.technicalSpecification[i]?.value?.value);
          ...
          if (
            ...
            $(".stickycol.specifications-battery-range")
              .next()
              .children()
              .eq(index)
              .text(vehicle.technicalSpecification[i]?.value?.value);

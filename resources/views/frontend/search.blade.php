...
@include('frontend.filter', ['class' => 'filter-search-section-cus'])

<section id="search-results">
    <div class="row">
        <div class="col-md-9">
            @include('frontend.filter-applied')
        </div>
        ...
        <div class="col-md-3 mb-5">
            <select name="order_by" id="search_sorting">
                ...
                <option value="pricing_desc"
                    {{ isset(request()->order_by) && request()->order_by == 'pricing_desc' ? 'selected="selected"' : ''}}>
                    Price high to low</option>
                <option value="pricing_asc"
                    {{ isset(request()->order_by) && request()->order_by == 'pricing_asc' ? 'selected="selected"' : (!isset(request()->order_by) ? 'selected="selected"' : '' )}}>
                    Price low to high</option>
                <option value="name_asc"
                    {{ isset(request()->order_by) && request()->order_by == 'name_asc' ? 'selected="selected"' : ''}}>A
                    to Z</option>
                <option value="name_desc"
                    {{ isset(request()->order_by) && request()->order_by == 'name_desc' ? 'selected="selected"' : ''}}>Z
                    to A</option>
            </select>
        </div>
    </div>
    <div class="row {{ $vehicles->isEmpty() ? 'justify-content-center' : '' }}">
        ...
        {{-- @if ($count%4 == 1)
                    <div class="col-12">
                      <img src="{{ asset('frontend/assets/images/Group 9311.png') }}" alt="">
    </div>
    @endif --}}

...
class UserController extends Controller
{
    ...
    public function update(Request $request)
    {
        ...
        if($request->has('image'))
            $user->image = $request->file('image')->storeAs('user', uniqid().uniqid().uniqid() . '.'. $request->file('image')->getClientOriginalExtension(), 'public');

        if($request->old_password)
            $user->password = bcrypt($request->password);

        if($request->username)
            $user->is_subscribed = $request->is_subscribed == 'on' ? 1 : 0;
    ...
    public function facebook()
    {
        $facebook_user = Socialite::driver('facebook')->fields([
            'name', 'first_name', 'last_name', 'email', 'gender', 'verified'
        ])->user();
